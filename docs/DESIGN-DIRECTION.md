...
- En sombre : fond de page `#11100F`, volet et cartes `#1A1917`, surfaces secondaires, champs et badges `#272624`, traits et séparateurs `#2E2E2B`, texte secondaire `#9A9893`, texte principal `#F1F0ED`. La barre supérieure conserve son niveau `#181715` et le cartouche de l’instance reprend le fond de page.
- En clair : fond de page `#FAF9F5`, volet et cartes `#FFFFFF`, surfaces secondaires, champs et badges `#F5F4F1`, traits et séparateurs `#E3E2DF`, texte secondaire `#717177`, texte principal `#181818`.
- Les accents d’état sont rouge `#F75D58` et vert `#53BE70` en sombre, rouge `#EF4444` et vert `#2EA957` en clair. L’accent de marque neutre reste `#CCC6BC` en sombre et `#58534C` en clair ; il distingue l’action principale et la route courante. Le texte sur accent utilise respectivement `#1C1B19` et `#FFFFFF`.
...
- Arrondis en pixels, indépendants de l’échelle typographique : 16 px pour les cartes, panneaux, KPI et incidents ; 12 px pour les filtres, champs de recherche, encarts d’espace et supports du logo ; 10 px pour les petits boutons, onglets et éléments de navigation ; 4 px pour les badges, compteurs et étiquettes mono. Les barres des graphiques n’arrondissent que leurs coins supérieurs, sur 2 px. Points d’état, avatars et contrôles circulaires restent entièrement ronds. Le signe Convergence conserve sa géométrie vectorielle.
- Les surfaces et l’espacement distinguent les cartes et les actions courantes sans leur dessiner un cadre. Les traits servent aux séparateurs de données, aux champs qui demandent une saisie et aux états de sélection ou de focus. La cloche des notifications et les autres commandes de la barre supérieure gardent leur cible sans bordure visible au repos.
- Typographie **Inter** pour les textes et titres ; **JetBrains Mono** pour les nombres, badges, versions, raccourcis et libellés techniques. Les deux familles variables, romaines et italiques, sont embarquées en WOFF2 avec leurs licences dans `web/static/fonts/`. Les valeurs numériques utilisent des chiffres tabulaires. Les capitales espacées ne servent jamais de décoration.
