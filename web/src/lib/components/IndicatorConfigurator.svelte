<script lang="ts">
  ...
  const brands: Partial<Record<Connector['kind'], BrandName>> = { zabbix: 'zabbix', uptime_kuma: 'uptime_kuma', patchmon: 'patchmon' };
  $effect(() => { void load(); });
...
<div class="scrim" role="presentation" onclick={(event) => event.currentTarget === event.target && !saving && onclose()}>
  <div class="modal indicator-modal" class:maximized role="dialog" aria-modal="true" aria-labelledby="indicator-config-title">
    <header>
      {#if brands[connector.kind]}<BrandMark name={brands[connector.kind]!} size={32} />{:else if connector.kind === 'proxmox'}<Icon name="server" size={32} />{/if}
