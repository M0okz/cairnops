<script lang="ts">
  ...
  async function inspect(event: SubmitEvent) {
    ...
    error = '';
    try {
      preview = await api<ZabbixPreview>('/api/v1/connectors/zabbix/preview', {
        method: 'POST',
  ...
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="connector-title">
    ...
    {:else if preview}
      ...
      <footer>
        ...
        <button class="btn primary" type="button" onclick={importHosts} disabled={busy || selected.length === 0}>
          {busy
            ? t('wizard.importing')
            : selected.length === 0
