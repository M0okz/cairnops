...
{#snippet incidentCard(item:typeof incidents[number])}
  <article class="p-incident-card">
    ...
    <div class="p-incident-actions"><span class="p-small">{item.since}</span>{#if acknowledged.includes(item.id)}<span class="p-ack"><Icon name="acknowledge" size={16}/>Acquitté</span>{:else}<button class="p-btn p-btn-small" onclick={()=>acknowledge(item.id)}>Acquitter<Icon name="acknowledge" size={18}/></button>{/if}</div>
  </article>
{/snippet}
...
{#snippet targetTable()}
  <section class="p-panel p-targets">
    ...
    <div class="p-table-wrap"><table class="p-table"><thead><tr><th>Cible</th><th>État de santé</th><th>Latence</th><th>Disponibilité <span>24 h</span></th><th>Source</th><th><span class="p-sr">Détail</span></th></tr></thead><tbody>
      ...
      {#if visible.length===0}<tr><td colspan="6"><div class="p-empty"><Icon name="search" size={32}/><strong>Aucune cible trouvée</strong><p>Essayez un autre nom ou retirez le filtre.</p><button class="p-btn" onclick={()=>{query='';filter='all';page=0;}}>Afficher les cibles</button></div></td></tr>{/if}
    </tbody></table></div>
...
{/snippet}
  ...
  <div class="p-app" data-variant={variant}>
    ...
    <main id="content" class="p-content" tabindex="-1">
      ...
      {#if section==='overview'}
        <div class="p-verdict"><span class="p-verdict-icon"><Icon name="incidents" size={22}/></span><div><strong>1 cible indisponible, 1 cible dégradée</strong><span>{pending>0?`${pending} incident${pending>1?'s':''} en attente de prise en charge.`:'Les deux incidents sont pris en charge.'} Dernières observations à 14:32.</span></div><button onclick={()=>navigate('incidents')}>Examiner <span>→</span></button></div>
      ...
      {:else if section==='connectors'}
        <div class="p-connector-grid">{#each [{name:'CairnOps',icon:'signal' as const,description:'Contrôles natifs et heartbeats',count:22},{name:'Uptime Kuma',icon:'health' as const,description:'Disponibilité et temps de réponse',count:18},{name:'Zabbix',icon:'server' as const,description:'Hôtes, signaux et indicateurs',count:24}] as connector}<article class="p-panel p-connector"><div class="p-connector-mark"><Icon name={connector.icon} size={28}/></div><span class="p-badge ok"><span class="p-dot"></span>Connecté</span><h2>{connector.name}</h2><p>{connector.description}</p><div><strong>{connector.count}</strong><span>sources configurées</span></div><button class="p-btn" onclick={()=>{navigate('targets');query=connector.name;}}>Voir les cibles <span>→</span></button></article>{/each}</div>
