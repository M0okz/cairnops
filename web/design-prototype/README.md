...
- Couleurs : graphite des consoles, pierre claire, cuivre des connexions, vert des états établis, ambre des avertissements, rouge des indisponibilités, bleu des informations contextuelles.
...
Fond graphite/pierre et accent cuivre conservés. Icônes de CairnOps réutilisées. Typographie système, chiffres tabulaires, titres 28 px, texte 13/14 px, contours fins et rayons de 6/8 px. Davantage d’air entre les groupes, alignements communs à l’intérieur. Les variantes de disposition et les espacements sont des propositions à reporter dans DESIGN-DIRECTION.md après choix.
...
1. Choisir la composition et ajuster le specimen.
2. Reporter les choix dans les jetons et la direction visuelle.
...
6. Produire le film de présentation définitif à partir du produit vérifié.
...
Première phase de la refonte, sur `codex/dashboard-design`. La composition reste à choisir avant de reporter la proposition dans les écrans métier. Le prototype est une entrée indépendante de l’application, avec ses propres fixtures ; il ne constitue pas une intégration de la refonte en production.
...
Le film dure 42,5 secondes : MP4 H.264, 1920 × 1080, 30 images/s, sans piste audio. `render-film.py` expose `--preview` pour les sept plans fixes. Les scripts Python nécessitent Playwright ; FFmpeg doit être disponible pour l’export. `CAIRNOPS_DESIGN_OUTPUT` permet de choisir le dossier de sortie. L’aperçu animé est accessible par `?film=1` et respecte la préférence de réduction des mouvements en attendant une lecture explicite.
