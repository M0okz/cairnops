...
with sync_playwright() as p:
    ...
    page.get_by_role('button',name='7 jours',exact=True).click()
    expect(page.get_by_role('button',name='7 jours',exact=True)).to_have_attribute('aria-pressed','true')
    report['checks'].append('Graphique : périodes et exploration clavier')
    page.get_by_role('button',name='À surveiller 2',exact=True).click()
    ...
    page.locator('.p-theme summary').click()
    page.get_by_role('button',name='Solaire',exact=True).click()
    expect(page.get_by_text('Choisis une ville pour activer le rythme solaire.',exact=False)).to_be_visible()
    page.get_by_label('Ville de référence').select_option('Paris')
    ...
    page.get_by_label('Ville de référence').select_option('Tromsø')
