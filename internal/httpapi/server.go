...
func NewServer(options ServerOptions) *http.Server {
	...
	if options.Connectors != nil && options.Identity != nil {
		...
		mux.Handle("POST /api/v1/connectors/proxmox/preview", identityHTTP.requireSameOrigin(identityHTTP.requireSession(identityHTTP.requireRole("administrator", http.HandlerFunc(handler.previewProxmox)))))
		mux.Handle("POST /api/v1/connectors/proxmox/certificate", identityHTTP.requireSameOrigin(identityHTTP.requireSession(identityHTTP.requireRole("administrator", http.HandlerFunc(handler.proxmoxCertificate)))))
	...
	if options.Incidents != nil && options.Identity != nil {
		...
		mux.Handle("POST /api/v1/incidents/{incidentID}/evidence/{evidenceID}/invalidation", identityHTTP.requireSameOrigin(identityHTTP.requireSession(identityHTTP.requireAnyRole([]string{"administrator", "operator"}, http.HandlerFunc(handler.invalidateEvidence)))))
	}
