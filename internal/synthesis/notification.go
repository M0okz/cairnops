...
var reportedCPUUtilization = regexp.MustCompile(`^Linux: High CPU utilization(?: \(over (?:[0-9]+(?:\.[0-9]+)?|\{\$CPU\.UTIL\.CRIT\})% for 5m\))?$`)

var reportedDiskLatency = regexp.MustCompile(`^Linux: [A-Za-z0-9_.-]+: Disk read/write request responses are too high(?: \(read > [0-9]+(?:\.[0-9]+)? ms for 15m or write > [0-9]+(?:\.[0-9]+)? ms for 15m\))?$`)
...
func notificationSourceTitle(s Situation, locale string) string {
	...
	if s.NatureScope == "connector" && strings.HasPrefix(s.NatureKey, "zabbix:") {
		...
		case reportedSystemLoad.MatchString(label):
			labels = [2]string{"Charge système moyenne élevée", "High average system load"}
		case reportedCPUUtilization.MatchString(label):
			labels = [2]string{"Utilisation CPU élevée", "High CPU utilization"}
		case reportedDiskLatency.MatchString(label):
			labels = [2]string{"Latence disque élevée", "High disk latency"}
		case reportedStorageSpace.MatchString(label):
			labels = [2]string{"Espace disque insuffisant", "Low disk space"}
		case reportedProxmoxFilesystem.MatchString(label):
			labels = [2]string{"Occupation disque élevée", "High filesystem usage"}
