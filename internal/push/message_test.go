...
func TestCompleteNotificationsUseTheApprovedThreeLineTemplate(t *testing.T) {
	for _, tt := range []struct {
		...
		{"approved storage notification", Delivery{NatureKey: "storage.latency", NatureScope: "canonical", NatureLabel: "Latence de stockage élevée", TargetName: "dmz-docker-01", Severity: "warning",
			Context: synthesis.Context{Fact: &alerttext.Fact{Kind: alerttext.DiskLatency, Resource: "sda"}, Sources: []string{"Zabbix"}}},
		...
		{"unrecognized source wording", Delivery{NatureKey: "zabbix:connector:custom", NatureScope: "connector", NatureLabel: "Linux: Load average is too high (per CPU load over 1.5 for 5m)", TargetName: "VictoriaLogs", Severity: "major", Context: synthesis.Context{Sources: []string{"Zabbix"}}},
			Presentation{"🟠 VictoriaLogs · majeur", "Linux: Load average is too high (per CPU load over 1.5 for 5m)\nvia Zabbix"}},
		{"resolution", Delivery{EventKind: "resolved", NatureKey: "availability", NatureScope: "canonical", TargetName: "Vikunja Todo", Severity: "critical", Context: synthesis.Context{DurationSeconds: 180, Sources: []string{"Uptime Kuma"}}},
			Presentation{"🟢 Vikunja Todo · résolu", "Indisponibilité\nRétabli après 3 min · via Uptime Kuma"}},
	} {
		...
		for _, mode := range []string{"discreet", "masked"} {
			for _, event := range []string{"firing", "incident_update", "resolved"} {
				delivery := Delivery{Locale: locale, NotificationContent: mode, EventKind: event,
					...
					Context: synthesis.Context{Sources: []string{"Secret Zabbix"}, TargetNames: []string{"Secret database"}}}
